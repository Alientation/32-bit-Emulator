...
void preprocessorTests() {
	...
	Process process = Process("-lib library1 -L " + REL_PATH_TO_FILES_FOLDER + 
            "libs -I " + REL_PATH_TO_FILES_FOLDER + 
            "include -o preprocessorTest " + REL_PATH_TO_FILES_FOLDER + 
            "preprocessorTest.basm");
	File file = File(REL_PATH_TO_FILES_FOLDER + "preprocessorTest." + SOURCE_EXTENSION);
...
void directoryTests() {
	...
	// test creating a file and its attributes
	Directory directory = Directory(REL_PATH_TO_FILES_FOLDER);
	...
	for (int i = 0; i < directories.size(); i++) {
		log(lgr::Logger::LogType::TEST, std::stringstream() << "Directory " << i 
                << ": " << directories[i].get_name());
	}
}
...
void fileReaderAll() {
	...
	// read from test file
	FileReader reader = FileReader(File(REL_PATH_TO_FILES_FOLDER + "readFromFile.txt"));
	std::string fileContents = reader.read_all();
	reader.close();
	log(lgr::Logger::LogType::TEST, std::stringstream() << "File Contents: \n" << fileContents);

	// write to test file
	FileWriter writer = FileWriter(File(REL_PATH_TO_FILES_FOLDER + "writeToFile.txt"));
...
void fileReaderBytes() {
	...
	// read byte one by one from test file
	File file = File(REL_PATH_TO_FILES_FOLDER + "readFromFile.txt");
