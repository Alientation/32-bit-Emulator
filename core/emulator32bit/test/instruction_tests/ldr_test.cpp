...
TEST(ldr, offset_reg) {
	...
	EXPECT_EQ(cpu->_x[0], 9) << "\'ldr x0, [x1, x2]\', where x1=PAGE_SIZE-3, x2=3 : should result in x0=9";
	EXPECT_EQ(cpu->_x[1], PAGE_SIZE - 3) << "operation should not change operand \'x1\'";
	EXPECT_EQ(cpu->_x[2], 3) << "operation should not change operand \'x2\'";
	EXPECT_EQ(test_bit(cpu->_pstate, N_FLAG), 0) << "operation should not cause N flag to be set";
	EXPECT_EQ(test_bit(cpu->_pstate, Z_FLAG), 0) << "operation should not cause Z flag to be set";
	EXPECT_EQ(test_bit(cpu->_pstate, C_FLAG), 0) << "operation should not cause C flag to be set";
	EXPECT_EQ(test_bit(cpu->_pstate, V_FLAG), 0) << "operation should not cause V flag to be set";
...
TEST(ldr, offset_reg_lsl) {
	...
	cpu->_pc = 0;
	cpu->_x[1] = PAGE_SIZE - 4;
	cpu->_x[2] = 2;
	cpu->set_NZCV(0, 0, 0, 0);

	cpu->run(1);

	EXPECT_EQ(cpu->_x[0], 9) << "\'ldr x0, [x1, x2, LSL #1]\', where x1=PAGE_SIZE-4, x2=2 : should result in x0=9";
	EXPECT_EQ(cpu->_x[1], PAGE_SIZE - 4) << "operation should not change operand \'x1\'";
	EXPECT_EQ(cpu->_x[2], 2) << "operation should not change operand \'x2\'";
	EXPECT_EQ(test_bit(cpu->_pstate, N_FLAG), 0) << "operation should not cause N flag to be set";
	EXPECT_EQ(test_bit(cpu->_pstate, Z_FLAG), 0) << "operation should not cause Z flag to be set";
	EXPECT_EQ(test_bit(cpu->_pstate, C_FLAG), 0) << "operation should not cause C flag to be set";
	EXPECT_EQ(test_bit(cpu->_pstate, V_FLAG), 0) << "operation should not cause V flag to be set";
...
TEST(ldr, offset_reg_lsr) {
	...
	EXPECT_EQ(cpu->_x[0], 9) << "\'ldr x0, [x1, x2, LSR #1]\', where x1=PAGE_SIZE-4, x2=8 : should result in x0=9";
	EXPECT_EQ(cpu->_x[1], PAGE_SIZE - 4) << "operation should not change operand \'x1\'";
	EXPECT_EQ(cpu->_x[2], 8) << "operation should not change operand \'x2\'";
	EXPECT_EQ(test_bit(cpu->_pstate, N_FLAG), 0) << "operation should not cause N flag to be set";
	EXPECT_EQ(test_bit(cpu->_pstate, Z_FLAG), 0) << "operation should not cause Z flag to be set";
	EXPECT_EQ(test_bit(cpu->_pstate, C_FLAG), 0) << "operation should not cause C flag to be set";
	EXPECT_EQ(test_bit(cpu->_pstate, V_FLAG), 0) << "operation should not cause V flag to be set";
