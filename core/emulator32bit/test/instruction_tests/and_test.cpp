...
TEST(and, register_and_register) {
	...
	EXPECT_EQ(cpu->_x[0], 0b0010) << "\'and x0, x1, 2\' : where x1=0b0011, x2=0b1010, should result in x0=0b0010";
...
TEST(and, negative_flag) {
	...
	EXPECT_EQ(cpu->_x[0], 1U<<31) << "\'and x0, x1, 2\' : where x1=~0, x2=1<<31, should result in x0=1<<31";
...
TEST(and, zero_flag) {
	...
	EXPECT_EQ(cpu->_x[0], 0) << "\'and x0, x1, 2\' : where x1=~0, x2=0, should result in x0=0";
	EXPECT_EQ(cpu->_x[1], ~0) << "operation should not alter operand register \'x1\'";
