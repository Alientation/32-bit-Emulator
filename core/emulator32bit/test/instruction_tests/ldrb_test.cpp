...
TEST(ldrb, offset) {
	...
	// ldrb x0, [x1, #3]
	cpu->system_bus.write_word(0, Emulator32bit::asm_format_m(Emulator32bit::_op_ldrb, false, 0, 1, 3, 0));
	cpu->_pc = 0;
...
TEST(ldrb, pre_indexed) {
	Emulator32bit *cpu = new Emulator32bit(1, 0, data, 1, 1);
	// ldrb x0, [x1, #3]!
	cpu->system_bus.write_word(0, Emulator32bit::asm_format_m(Emulator32bit::_op_ldrb, false, 0, 1, 3, 1));
	cpu->_pc = 0;
	cpu->_x[1] = PAGE_SIZE - 3;
	cpu->set_NZCV(0, 0, 0, 0);
...
TEST(ldrb, post_indexed) {
	Emulator32bit *cpu = new Emulator32bit(1, 0, data, 1, 1);
	// ldrb x0, [x1, #3]!
	cpu->system_bus.write_word(0, Emulator32bit::asm_format_m(Emulator32bit::_op_ldrb, false, 0, 1, 3, 2));
	cpu->_pc = 0;
	cpu->_x[1] = PAGE_SIZE;
	cpu->set_NZCV(0, 0, 0, 0);
