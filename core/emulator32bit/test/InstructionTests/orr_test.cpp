...
TEST(orr, register_and_register) {
	...
	// x1: 0b0011
	// x2: 0b1010
	cpu->system_bus.writeWord(0, Emulator32bit::asm_format_o(Emulator32bit::_op_orr, true, 0, 1, 2, 0, 0));
	cpu->_pc = 0;
	cpu->_x[1] = 0b0011;
	cpu->_x[2] = 0b1010;
	...
	EXPECT_EQ(cpu->_x[0], 0b1011) << "\'orr x0, x1, 2\' : where x1=0b0011, x2=0b1010, should result in x0=0b1011";
...
TEST(orr, negative_flag) {
	...
	EXPECT_EQ(cpu->_x[0], 1U<<31) << "\'orr x0, x1, 2\' : where x1=0, x2=1<<31, should result in x0=1<<31";
...
TEST(orr, zero_flag) {
	...
	EXPECT_EQ(cpu->_x[0], 0) << "\'orr x0, x1, 2\' : where x1=0, x2=0, should result in x0=0";
	EXPECT_EQ(cpu->_x[1], 0) << "operation should not alter operand register \'x1\'";
