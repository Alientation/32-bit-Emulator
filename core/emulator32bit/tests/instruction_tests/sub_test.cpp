...
TEST_F (EmulatorFixture, sub_register_sub_register)
{
    ...
    cpu->system_bus->write_word (0,
                                 Emulator32bit::asm_format_o (Emulator32bit::_op_sub, false, 0, 1,
...
TEST_F (EmulatorFixture, sub_negative_flag)
{
    ...
    // x1: 1
    // x2: 2
    cpu->system_bus->write_word (0, Emulator32bit::asm_format_o (Emulator32bit::_op_sub, true, 0, 1,
                                                                 2, Emulator32bit::SHIFT_LSL, 0));
    cpu->set_pc (0);
    cpu->write_reg (1, 1);
    cpu->write_reg (2, 2);
...
TEST_F (EmulatorFixture, sub_zero_flag)
{
    ...
    EXPECT_EQ (cpu->read_reg (0), 0)
        << "\'sub x0, x1, x2\' : where x1=1, x2=1, should result in x0=0";
    EXPECT_EQ (cpu->read_reg (1), 1) << "operation should not alter operand register \'x1\'";
