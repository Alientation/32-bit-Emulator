...
TEST_F(EmulatorFixture, and_register_and_register) {
    cpu = new(cpu) Emulator32bit(1, 0, {}, 0, 1);
    // and x0, x1, x2
    // x1: 0b0011
    // x2: 0b1010
    cpu->system_bus.write_word(0, Emulator32bit::asm_format_o(Emulator32bit::_op_and, true, 0, 1, 2, Emulator32bit::SHIFT_LSL, 0));
    cpu->set_pc(0);
    ...
    EXPECT_EQ(cpu->read_reg(0), 0b0010) << "\'and x0, x1, 2\' : where x1=0b0011, x2=0b1010, should result in x0=0b0010";
...
TEST_F(EmulatorFixture, and_negative_flag) {
    cpu = new(cpu) Emulator32bit(1, 0, {}, 0, 1);
    // and x0, x1, x2
    // x1: ~0
    // x2: 1<<31
    ...
    EXPECT_EQ(cpu->read_reg(0), 1U<<31) << "\'and x0, x1, 2\' : where x1=~0, x2=1<<31, should result in x0=1<<31";
...
TEST_F(EmulatorFixture, and_zero_flag) {
    cpu = new(cpu) Emulator32bit(1, 0, {}, 0, 1);
    // and x0, x1, x2
    // x1: ~0
    // x2: 0
    ...
    EXPECT_EQ(cpu->read_reg(0), 0) << "\'and x0, x1, 2\' : where x1=~0, x2=0, should result in x0=0";
    EXPECT_EQ(cpu->read_reg(1), ~0) << "operation should not alter operand register \'x1\'";
