...
TEST_F (EmulatorFixture, mul_register_mul_immediate)
{
    ...
    cpu->system_bus->write_word (
        0, Emulator32bit::asm_format_o (Emulator32bit::_op_mul, false, 0, 1, 9));
    cpu->set_pc (0);
    cpu->write_reg (1, 2);
...
TEST_F (EmulatorFixture, mul_register_mul_register)
{
    ...
    cpu->system_bus->write_word (0,
                                 Emulator32bit::asm_format_o (Emulator32bit::_op_mul, false, 0, 1,
...
TEST_F (EmulatorFixture, mul_negative_flag)
{
    ...
    // x1: -2
    // x2: 4
    cpu->system_bus->write_word (0, Emulator32bit::asm_format_o (Emulator32bit::_op_mul, true, 0, 1,
                                                                 2, Emulator32bit::SHIFT_LSL, 0));
    cpu->set_pc (0);
    cpu->write_reg (1, -2);
    cpu->write_reg (2, 4);
...
TEST_F (EmulatorFixture, mul_zero_flag)
{
    ...
    EXPECT_EQ (cpu->read_reg (0), 0)
        << "\'mul x0, x1, x2\' : where x1=0, x2=4, should result in x0=0";
