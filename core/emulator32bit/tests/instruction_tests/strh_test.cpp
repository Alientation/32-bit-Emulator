...
TEST_F(EmulatorFixture, strh_offset) {
    ...
    EXPECT_EQ(cpu->system_bus.read_hword(8), 9) << "\'strh x0, [x1, #3]\', where x0=9, x1=5 : should result in 2 byte value of 9 to be written at address 8";
...
TEST_F(EmulatorFixture, strh_post_indexed) {
    ...
    EXPECT_EQ(cpu->system_bus.read_hword(8), 9) << "\'strh x0, [x1], #3\', where x0=9, x1=8 : should result in 2 byte value of 9 to be written at address 8";
    EXPECT_EQ(cpu->read_reg(0), 9) << "operation should not change operand \'x0\'";
