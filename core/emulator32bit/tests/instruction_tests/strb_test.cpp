...
TEST_F (EmulatorFixture, strb_offset)
{
    cpu = new (cpu) Emulator32bit (1, 0, {}, 0, 1);
    // strb x0, [x1, #3]
    cpu->system_bus->write_word (0, Emulator32bit::asm_format_m (Emulator32bit::_op_strb, false, 0,
    ...
    EXPECT_EQ (cpu->system_bus->read_byte (8), 9)
        << "\'strb x0, [x1, #3]\', where x0=9, x1=5 : should result in 1 byte value of 9 to be "
...
TEST_F (EmulatorFixture, strb_pre_indexed)
{
    ...
    cpu->system_bus->write_word (0,
                                 Emulator32bit::asm_format_m (Emulator32bit::_op_strb, false, 0, 1,
...
TEST_F (EmulatorFixture, strb_post_indexed)
{
    ...
    EXPECT_EQ (cpu->system_bus->read_byte (8), 9)
        << "\'strb x0, [x1], #3\', where x0=9, x1=8 : should result in 1 byte value of 9 to be "
           "written at address 8";
    EXPECT_EQ (cpu->read_reg (0), 9) << "operation should not change operand \'x0\'";
