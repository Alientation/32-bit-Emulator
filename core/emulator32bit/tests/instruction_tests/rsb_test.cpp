...
TEST_F (EmulatorFixture, rsb_register_rsb_register)
{
    ...
    cpu->system_bus->write_word (0,
                                 Emulator32bit::asm_format_o (Emulator32bit::_op_rsb, false, 0, 1,
...
TEST_F (EmulatorFixture, rsb_negative_flag)
{
    ...
    // x1: 2
    // x2: 1
    cpu->system_bus->write_word (0, Emulator32bit::asm_format_o (Emulator32bit::_op_rsb, true, 0, 1,
                                                                 2, Emulator32bit::SHIFT_LSL, 0));
    cpu->set_pc (0);
    cpu->write_reg (1, 2);
    cpu->write_reg (2, 1);
...
TEST_F (EmulatorFixture, rsb_zero_flag)
{
    ...
    EXPECT_EQ (cpu->read_reg (0), 0)
        << "\'rsb x0, x1, x2\' : where x1=1, x2=1, should result in x0=0";
