...
void VirtualMemory::set_process(long long pid) {
	...
	lgr::log(lgr::Logger::LogType::DEBUG, std::stringstream() << "Setting memory map to process " << std::to_string(pid));
}
...
void VirtualMemory::add_page(word vpage) {
	...
	lgr::log(lgr::Logger::LogType::DEBUG, std::stringstream() << "Adding virtual page " <<
			std::to_string(vpage) << " to process " << std::to_string(m_cur_ptable->pid));
}
...
void VirtualMemory::remove_page(long long pid, word vpage) {
	...
	if (ptable->entries.find(vpage) == ptable->entries.end()) {
		lgr::log(lgr::Logger::LogType::ERROR, std::stringstream() << "Cannot remove virtual page "
				<< std::to_string(vpage) << " because it is not mapped in process " << std::to_string(pid));
		return;
	}
	...
	if (entry->disk) {
		...
		lgr::log(lgr::Logger::LogType::DEBUG, std::stringstream() << "Returning disk page "
				<< std::to_string(entry->diskpage) << " corresponding to virtual page " << std::to_string(vpage));
	} else {
		...
		lgr::log(lgr::Logger::LogType::DEBUG, std::stringstream() << "Returning physical page "
				<< std::to_string(entry->ppage) << " corresponding to virtual page " << std::to_string(vpage));
	}
...
void VirtualMemory::check_vm() {
	for (std::pair<word,PageTableEntry*> pair : m_physical_memory_map) {
		lgr::log(lgr::Logger::LogType::DEBUG, std::stringstream() << "Checking physical page " << std::to_string(pair.first));
		EXPECT_TRUE(pair.first == pair.second->ppage, lgr::Logger::LogType::ERROR, std::stringstream() << "Expected physical memory to match");
		EXPECT_TRUE(pair.second->disk == false, lgr::Logger::LogType::ERROR, std::stringstream() << "Expected physical memory to not be on disk");
	}

	for (std::pair<long long, PageTable*> pair : m_process_ptable_map) {
		lgr::log(lgr::Logger::LogType::DEBUG, std::stringstream() << "Checking process " << std::to_string(pair.first));
		EXPECT_TRUE(pair.second->pid == pair.first, lgr::Logger::LogType::ERROR, std::stringstream() << "Expected Process ID to match");
		for (std::pair<word,PageTableEntry*> entry : pair.second->entries) {
			lgr::log(lgr::Logger::LogType::DEBUG, std::stringstream() << "Checking page entry at vpage " << std::to_string(entry.first));
			EXPECT_TRUE(entry.first == entry.second->vpage, lgr::Logger::LogType::ERROR, std::stringstream() << "Expected virtual memory to match");

			if (!entry.second->disk) {
				EXPECT_TRUE(m_physical_memory_map.at(entry.second->ppage) == entry.second, lgr::Logger::LogType::ERROR, std::stringstream() << "Expected page entry to match");
			}
		}
	}
}
...
word VirtualMemory::access_page(word vpage, Exception& exception) {
	...
	if (!entry->disk) {				/* Not on disk */
		lgr::log(lgr::Logger::LogType::DEBUG, std::stringstream() << "accessing virtual page (NOT ON DISK) "
	...
	if (!m_freelist.can_fit(1)) {
		...
		lgr::log(lgr::Logger::LogType::DEBUG, std::stringstream() << "Evicting physical page "
				<< std::to_string(ppage_to_disk) << " to disk");
		...
		if (fbl_exception.type != FreeBlockList::Exception::Type::AOK) {
			lgr::log(lgr::Logger::LogType::ERROR, std::stringstream() << "FBL Exception");
		}
	...
	lgr::log(lgr::Logger::LogType::DEBUG, std::stringstream() << "Disk Fetch from page "
				<< std::to_string(entry->diskpage) << " to physical page " << std::to_string(ppage));

	if (disk_read_exception.type != Disk::ReadException::Type::AOK) {
		exception.type = Exception::Type::DISK_FETCH_FAILED;
		lgr::log(lgr::Logger::LogType::ERROR, std::stringstream() << "Disk fetch failed for process " << std::to_string(m_cur_ptable->pid));
		return 0;
	}

	m_disk.return_page(entry->diskpage, fbl_exception);
	if (fbl_exception.type != FreeBlockList::Exception::Type::AOK) {
		lgr::log(lgr::Logger::LogType::ERROR, std::stringstream() << "Failed to return disk page to free list.");
	}
	...
	lgr::log(lgr::Logger::LogType::DEBUG, std::stringstream() << "accessing virtual page " << std::to_string(vpage)
			<< " (maps to " << std::to_string(ppage) << ")" << " of process " << std::to_string(m_cur_ptable->pid));
	return entry->ppage;
}
...
void VirtualMemory::begin_process(long long pid, word alloc_mem_begin, word alloc_mem_end) {
	...
	lgr::log(lgr::Logger::LogType::DEBUG, std::stringstream() << "Beginning process " << std::to_string(m_cur_ptable->pid));
}
...
void VirtualMemory::end_process(long long pid) {
	...
	lgr::log(lgr::Logger::LogType::DEBUG, std::stringstream() << "Ending process " << std::to_string(pid));
}
...
word VirtualMemory::map_address(word address, Exception& exception) {
	...
	lgr::log(lgr::Logger::LogType::DEBUG, std::stringstream() << "Mapping address " << std::to_string(address) << ".");

	word vpage = address >> PAGE_PSIZE;

	if (m_cur_ptable->entries.find(vpage) == m_cur_ptable->entries.end()) {
		lgr::log(lgr::Logger::LogType::DEBUG, std::stringstream() << "Adding virtual memory page " << std::to_string(vpage) << ".");
		add_page(vpage);
	}

	word ppage = access_page(vpage, exception);
	lgr::log(lgr::Logger::LogType::DEBUG, std::stringstream() << "Accessing virtual memory page " << std::to_string(vpage)
			<< " which is physical page " << std::to_string(ppage) << ".");
	return (ppage << PAGE_PSIZE) + (address & (PAGE_SIZE-1));
}

void VirtualMemory::check_lru() {
	lgr::log(lgr::Logger::LogType::DEBUG, std::stringstream() << "Checking LRU");

	if (m_lru_head == nullptr || m_lru_tail == nullptr) {
		EXPECT_TRUE(m_lru_head == m_lru_tail, lgr::Logger::LogType::ERROR, std::stringstream() << "Expected list to be empty");
		EXPECT_TRUE(m_lru_map.size() == 0, lgr::Logger::LogType::ERROR, std::stringstream() << "Expected lru map to be empty since list is empty");
		return;
	}
	...
	while (cur->next != nullptr) {
		EXPECT_TRUE(cur == cur->next->prev, lgr::Logger::LogType::ERROR, std::stringstream() << "Expected the next node's previous to point back");
		EXPECT_TRUE(m_lru_map.at(cur->ppage) == cur, lgr::Logger::LogType::ERROR, std::stringstream() << "Expected lru map to match list");
		mapped_ppages.insert(cur->ppage);
		cur = cur->next;
	}
	mapped_ppages.insert(cur->ppage);

	EXPECT_TRUE(cur == m_lru_tail, lgr::Logger::LogType::ERROR, std::stringstream() << "Expected list to end at tail");

	for (std::pair<word,LRU_Node*> pair : m_lru_map) {
		EXPECT_TRUE(mapped_ppages.find(pair.first) != mapped_ppages.end(), lgr::Logger::LogType::ERROR, std::stringstream()
				<< "Expected LRU map to correspond to the LRU list. " << std::to_string(pair.first) << " is not in the lru list");
	}
}
