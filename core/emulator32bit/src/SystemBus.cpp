...
word SystemBus::map_address(word address, VirtualMemory::Exception& exception) {
	...
	if (exception.type == VirtualMemory::Exception::Type::DISK_RETURN_AND_FETCH_SUCCESS) {
		...
		if (bus_exception.type != Exception::Type::AOK) {
			lgr::log(lgr::Logger::LogType::ERROR, std::stringstream() << "Bus is not in an AOK state");
			return 0;
		}
		...
		if (mem_read_exception.type != Memory::ReadException::Type::AOK) {
			lgr::log(lgr::Logger::LogType::ERROR, std::stringstream() << "Memory read failed");
			return 0;
		}
		...
		if (disk_write_exception.type != Disk::WriteException::Type::AOK) {
			lgr::log(lgr::Logger::LogType::ERROR, std::stringstream() << "Disk write failed");
			return 0;
		}

		lgr::log(lgr::Logger::LogType::DEBUG, std::stringstream() << "Writing physical page "
				<< std::to_string(exception.ppage_return) << " to disk page " << std::to_string(exception.disk_page_return));
	}
	...
	if (exception.type == VirtualMemory::Exception::Type::DISK_FETCH_SUCCESS) {
		...
		if (mem_write_exception.type != Memory::WriteException::Type::AOK) {
			lgr::log(lgr::Logger::LogType::ERROR, std::stringstream() << "Memory write failed");
			return 0;
		}

		lgr::log(lgr::Logger::LogType::DEBUG, std::stringstream() << "Reading physical page "
...
word SystemBus::map_read_address(word address, SystemBus::Exception &bus_exception, Memory::ReadException &mem_exception) {
...
word SystemBus::map_write_address(word address, SystemBus::Exception &bus_exception, Memory::WriteException &mem_exception) {
	VirtualMemory::Exception vm_exception;
...
dword SystemBus::read_val(word address, int n_bytes, Exception &bus_exception, Memory::ReadException &mem_exception, bool memory_mapped) {
	dword val = 0;
...
byte SystemBus::read_byte(word address, Exception &bus_exception, Memory::ReadException &mem_exception, bool memory_mapped) {
	return read_val(address, 1, bus_exception, mem_exception, memory_mapped);
}

hword SystemBus::read_hword(word address, Exception &bus_exception, Memory::ReadException &mem_exception, bool memory_mapped) {
	return read_val(address, 2, bus_exception, mem_exception, memory_mapped);
}

word SystemBus::read_word(word address, Exception &bus_exception, Memory::ReadException &mem_exception, bool memory_mapped) {
	return read_val(address, 4, bus_exception, mem_exception, memory_mapped);
}

void SystemBus::write_val(word address, dword val, int n_bytes, Exception &bus_exception, Memory::WriteException &mem_exception, bool memory_mapped) {
	for (int i = 0; i < n_bytes; i++) {
		word real_adr = address;
		if (memory_mapped) {
...
void SystemBus::write_byte(word address, byte data, Exception &bus_exception, Memory::WriteException &mem_exception, bool memory_mapped) {
	write_val(address, data, 1, bus_exception, mem_exception, memory_mapped);
}

void SystemBus::write_hword(word address, hword data, Exception &bus_exception, Memory::WriteException &mem_exception, bool memory_mapped) {
	write_val(address, data, 2, bus_exception, mem_exception, memory_mapped);
}

void SystemBus::write_word(word address, word data, Exception &bus_exception, Memory::WriteException &mem_exception, bool memory_mapped) {
