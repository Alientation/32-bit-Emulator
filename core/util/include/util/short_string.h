...
class ShortString
{
  public:
    ...
    inline ShortString &substring (const U32 pos, const U32 len)
    {
        ...
        const U32 clamped_new_len = (new_len > kMaxLength) ? kMaxLength : new_len;
        memmove (m_str, m_str + pos, clamped_new_len);
        m_len = clamped_new_len;
        m_str[clamped_new_len] = '\0';
        return *this;
    }
    ...
    inline U32 find (const ShortString<kMaxPatternLength> &pattern) const
    {
    ...
    ///
    template<U32 kMaxPatternLength>
    inline U32 find_from (const ShortString<kMaxPatternLength> &pattern, const U32 pos) const
    {
        if (UNLIKELY (pattern.len () > m_len))
        {
            return U32 (-1);
        }
    ...
    ///
    /// @brief              Return two substrings split off from the current string.
    ///                     Current string remains unmodified.
    ///
    /// @param pos          Position of the substring.
    /// @param len          Length of the substring.
    ///
    /// @return             The substring removed from the current string.
    ///
    inline ShortStringPair split (const U32 pos, const U32 len = 0) const
    {
        if (UNLIKELY (pos >= m_len))
        {
            return {.str1 = *this, .str2 = ShortString ("")};
        }

        ShortString str1;
        memcpy (str1.m_str, m_str, pos);
        str1.m_len = pos;

        ShortString str2;
        const U32 remaining_len = (pos + len > m_len) ? 0 : m_len - pos - len;
        str2.m_len = remaining_len;
        memcpy (str2.m_str, m_str + pos + len, remaining_len);
        return {.str1 = str1, .str2 = str2};
    }

    ///
    /// @brief              Return two substrings split at the first pattern substring.
    ///                     Current string remains unmodified. If no pattern substring exists,
    ///                     returns the entire string as the first substring.
    ///
    /// @tparam kMaxPatternLength   Max length of pattern string.
    ///
    /// @param pattern      Pattern string.
    ///
    /// @return             Two substrings split at pattern substring.
    ///
    template<U32 kMaxPatternLength>
    inline ShortStringPair split (const ShortString<kMaxPatternLength> &pattern) const
    {
        return split (find (pattern), pattern.len ());
    }

    ///
    /// @brief              Return two substrings split at the first pattern substring from pos.
    ///                     Current string remains unmodified. If no pattern substring exists,
    ///                     returns the entire string as the first substring.
    ///
    /// @tparam kMaxPatternLength   Max length of pattern string.
    ///
    /// @param pattern      Pattern string.
    /// @param pos          Position to start search for pattern string.
    ///
    /// @return             Two substrings split at pattern substring.
    ///
    template<U32 kMaxPatternLength>
    inline ShortStringPair split_from (const ShortString<kMaxPatternLength> &pattern,
                                       const U32 pos) const
    {
  ...
  private:
    ...
    static_assert (kMaxLength < U32 (-1), "kMaxLength too large, would overflow array size");
