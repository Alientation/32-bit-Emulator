...
static token_t *nxttok (parser_data_t *parser)
{
    if (parser->tok_i < parser->lexer->tokarr.tok_cnt)
    {
        return &parser->lexer->tokarr.toks[parser->tok_i++];
    }
...
static bool istok (parser_data_t *parser, tokentype_t type)
{
    ...
    token_t *tok = &parser->lexer->tokarr.toks[parser->tok_i];
...
static tokentype_t toktype (parser_data_t *parser)
{
    ...
    return parser->lexer->tokarr.toks[parser->tok_i].type;
}
